...
const ServiceCard = ({ 
  ...
  return (
    <div 
      className={`${color} rounded-xl shadow-lg transition-all duration-500 transform relative overflow-hidden group ${className}`}
      style={{ 
    ...
    >
      ...
      <div className="relative z-10 p-6">
        ...
        <h3 className="font-display font-bold text-2xl mb-3 text-white drop-shadow-md group-hover:translate-x-1 transition-transform duration-300">{title}</h3>
        <p className="text-white/90 mb-5 drop-shadow-sm">{description}</p>
        
        <div className="space-y-3 mb-6">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="flex items-center text-white/90 transition-transform duration-300"
