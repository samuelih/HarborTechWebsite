  ...
  return (
    <div 
      className={`relative overflow-hidden backdrop-blur-sm transition-all duration-500 rounded-2xl ${
        isPopular ? 'bg-white shadow-2xl' : 'bg-white/90 shadow-lg'
      }`}
      style={{ 
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div 
          ...
          style={{ 
            opacity: isHovered && !isPopular ? 0.8 : 1,
            background: isPopular ? 'linear-gradient(to bottom right, rgba(255,255,255,0.4), rgba(255,255,255,0.1))' : 'none'
          }}
        ></div>
      </div>
        ...
        <div className="text-center mb-8 relative">
          {icon && (
            <div className="w-16 h-16 mx-auto mb-4 relative">
              <div 
                className={`absolute inset-0 rounded-full ${isPopular ? 'bg-primary-100' : 'bg-neutral-gull/50'} transition-transform duration-500`}
          ...
          <p className="text-secondary-navy/70 mb-4 text-sm">{subtitle}</p>
          ...
          >
            <div className="text-4xl font-display font-black bg-gradient-to-r from-primary-navy to-primary-700 bg-clip-text text-transparent">
              {price}
              <span className="text-base font-normal text-secondary-navy/60 ml-1">flat fee</span>
            </div>
          ...
          {features.map((feature, index) => (
            ...
            >
              <span className={`w-5 h-5 rounded-full flex items-center justify-center mr-3 mt-0.5 flex-shrink-0 transition-colors duration-300 ${
                isPopular ? 'bg-primary-100' : 'bg-neutral-gull/80'
              } ${isHovered ? (isPopular ? 'bg-primary-200' : 'bg-neutral-gull') : ''}`}>
                <svg 
                  className={`w-3 h-3 transition-colors duration-300 ${
                    isPopular ? 'text-primary-700' : 'text-primary-500'
                  }`}
                  fill="none" 
                  stroke="currentColor" 
                  viewBox="0 0 24 24" 
              ...
              <span className="text-secondary-navy group-hover:text-primary-navy transition-colors duration-300">{feature}</span>
            </li>
          ))}
        </ul>
          ...
          <Link 
            ...
            className={`w-full text-center py-3.5 px-6 rounded-lg font-semibold relative overflow-hidden flex items-center justify-center group transition-all duration-500 ${
              isPopular 
                ...
                : 'border-2 border-primary-500 text-primary-700 hover:text-primary-500 bg-white'
            }`}
          >
